...
export function FloatingButtonGroup({ className, children }: FloatingButtonGroupProps) {
  ...
  useEffect(() => {
    ...
    const findHeadings = () => {
      ...
      const headingElements = elements.map((element) => {
        ...
        return null
      }).filter(Boolean) as Array<{id: string, text: string, level: number, element: HTMLElement}>
  ...
  const githubLink = SOCIAL_LINKS.find((link: SocialLink) => link.id === 'github')?.path || '#'
  const linkedinLink = SOCIAL_LINKS.find((link: SocialLink) => link.id === 'linkedin')?.path || '#'
  ...
  return (
    <FloatingButtonContext.Provider value={contextValue}>
      <div ref={tocRef} className={cn('fixed top-[var(--navbar-height)] right-4 sm:right-6 md:right-8 lg:right-12 z-50 flex flex-col items-end gap-4', className)}>
        {children}
      </div>
...
export function TocButton() {
  ...
  return (
    ...
    >
      {activeButton === 'toc' ? <XIcon className="h-5 w-5 sm:h-5 sm:w-5 md:h-6 md:w-6 lg:h-7 lg:w-7" /> : <Menu className="h-5 w-5 sm:h-5 sm:w-5 md:h-6 md:w-6 lg:h-7 lg:w-7" />}
    </Button>
  )
}
...
export function ContactButton() {
  ...
  return (
    ...
    >
      {activeButton === 'contact' ? <XIcon className="h-5 w-5 sm:h-5 sm:w-5 md:h-6 md:w-6 lg:h-7 lg:w-7" /> : <UserIcon className="h-5 w-5 sm:h-5 sm:w-5 md:h-6 md:w-6 lg:h-7 lg:w-7" />}
    </Button>
  )
}
...
export function ShareButton() {
  ...
  return (
    ...
    >
      {activeButton === 'share' ? <XIcon className="h-5 w-5 sm:h-5 sm:w-5 md:h-6 md:w-6 lg:h-7 lg:w-7" /> : <Share2Icon className="h-5 w-5 sm:h-5 sm:w-5 md:h-6 md:w-6 lg:h-7 lg:w-7" />}
    </Button>
  )
}
...
export function ContactMenu() {
  ...
  return (
    <div className="flex flex-col gap-4 mt-4 animate-in slide-in-from-top duration-200">
      <ContentCard 
        title="Jiin Seok" 
        icon={<div className="bg-primary rounded-full h-5 w-5" aria-hidden="true"></div>}
      ...
      >
        <address className="space-y-2 text-sm text-muted-foreground not-italic">
          <div className="flex items-center gap-2">
            ...
            <a href="mailto:seokjiin1073@gmail.com" className="hover:underline">seokjiin1073@gmail.com</a>
          </div>
          <div className="flex items-center gap-2">
  ...
  return (
    <div className="flex flex-col gap-4 mt-4 animate-in slide-in-from-top duration-200">
      <ContentCard 
        ...
        className="w-64 sm:w-64 md:w-72 lg:w-80 shadow-lg border border-border/50 backdrop-blur-sm bg-background/90 p-1"
      >
        <nav className="pr-3 -mr-2 flex flex-col" aria-label="Table of Contents">
          <ul className="space-y-2">
            {headings.map((heading) => (
              ...
              >
                ...
                >
                  {activeId === heading.id && (
                    <span className="w-1 h-4 bg-primary rounded-full mr-2" aria-hidden="true" />
                  )}
                  {heading.text}
                </a>
              </li>
